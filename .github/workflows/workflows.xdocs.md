...
children: []
files:
  ci.yml: Installs Bun dependencies, type-checks, tests, builds, and compiles the local executable plus release matrix.
...
The CI workflow verifies RunX on every main-branch push and pull request. The
Publish workflow runs only for `@guiho/runx@*` tags, retains the `production`
