...
files:
  ...
  catalog.go: Implements real manifest-backed check, aligned list, numeric-index-aware describe/run, safe-default interactive confirmation with exact retries, and init commands with deterministic text and JSON.
  ...
  root_test.go: Exercises bootstrap/exclusions, version, welcome, every help scope, aliases, aligned lists, numeric index selection, interactive and noninteractive confirmation, exact retries, real manifests, no-spawn inspection, argument forwarding, full agent actions, and initialization.
documents: {}
tags:
  - cli
  - go
