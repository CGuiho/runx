...
files:
  ...
  install.ps1: Stages under $HOME/.guiho/.temp/, hashes with the module-independent .NET SHA-256 API, verifies the release manifest, self-tests before activation, writes current.json as BOM-free UTF-8, installs the canonical launcher/payload layout, rolls back on failure, and configures user PATH idempotently.
  install_ps1_test.go: Verifies script contracts and syntax, then runs native Windows PowerShell clean install, injected rollback, same-version reinstall, and a whole-release upgrade using the standard non-self-hashing checksum manifest.
